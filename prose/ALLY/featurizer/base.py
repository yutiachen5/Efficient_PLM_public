...
def get_featurizer(featurizer, *args, **kwargs):
    import prose.ALLY.featurizer as featurizers

    return getattr(featurizers, featurizer)(*args, **kwargs)
...
class Featurizer:
    ...
    def write_to_disk(self, seq_list: T.List[str], verbose: bool = True) -> None:
        print(f"Writing {self.name} features to {self.path}")
