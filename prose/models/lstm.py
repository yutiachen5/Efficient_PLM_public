...
class SkipLSTM(nn.Module):
    ...
    def to_one_hot(self, x):
        ...
        if packed:
            ...
            if x.dim() == 1:  # Single vector case: [seq_len]
                one_hot = x.new(x.size(0), self.nin).float().zero_()
                one_hot.scatter_(1, x.unsqueeze(1), 1)
            else:  # Batch case: [batch_size, seq_len]
    ...
    def forward(self, x):
        ...
        if type(x) is PackedSequence:
            ...
            if x.dim() == 1:  # Handle single vector case: [seq_len]
                h = torch.cat([z.squeeze(0) for z in hs], 1)  # Squeeze batch dimension
                z = self.proj(h)
            else:
                h = torch.cat([z for z in hs], 2)
        ...
        return z, h # h: embedding [seq_len, hidden_dim] = [seq_len, 3093]
